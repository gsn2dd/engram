...
The attach point is an **MCP endpoint** (SSE transport, for the widest client compatibility): spin up the container and point any MCP-capable agent (OpenClaw, Claude Desktop, your own) at `http://<host>:8080/sse` — it has a brain, no glue code. Set `ENGRAM_MCP_TOKEN` (24+ chars) and the endpoint requires `Authorization: Bearer <token>` on every request — do this before binding it anywhere beyond loopback, and put TLS or a tunnel in front so the token doesn't travel in cleartext. Without the variable the endpoint is open, which is only safe on localhost. The server exposes six tools — `remember`, `recall`, `recall_with_associations`, `supersede`, `remember_json` (fold a whole JSON blob into recallable memories, one per leaf), and `recall_json` (reassemble a folded blob back into one object) — backed by everything above. Run one brain per agent, or share one across a fleet. *(You can also drive it directly via the Python API / CLI, shown below.)*
